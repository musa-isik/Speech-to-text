 ...
 * @author m-xz9
 */
public class VoskService implements Runnable {

    private Model model = null;
    private MicrophoneService microphoneService;
    private BlockingQueue<String> resultQueue = new LinkedBlockingQueue<>();
    public VoskService(String model_path, MicrophoneService microphoneService, BlockingQueue<String> resultQueue) {
            ...
            while (!Thread.currentThread().isInterrupted()) {
                ...
                if (bytesRead > 0) {
                    if (recognizer.acceptWaveForm(buffer, bytesRead)) {
                        resultQueue.put(recognizer.getResult().split("\"text\"\\s*:\\s*")[1].split("\"")[1]);
                    }
                }
            }
        } catch (Exception ex) {
